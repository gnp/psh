...
=item * Extended Completion code for Perl (supports $#foo, $foo->{key,
$foo->method, subroutine without &, perl reserved symbol) - Hiroo Hayashi
[2000-03-11]

=item * Executable completion now even after initial '!' for shell escape - warp [2000-03-11]

=item * Some minimal status code testing and error handling for user input - warp [2000-03-11]

=item * Understands environment variable IGNOREEOF - warp [2000-03-11]

=item * Accepts the -i command line switch and ignores it (bash compatibility) - warp [2000-03-11]

=item * New builtin 'history' -  warp [2000-03-11]

=item * If the line you entered matches the last line it won't be added to the history (maybe make this configurable?) - warp [2000-03-11]

=item * Start of a new package "Psh::Completion::Defaults", which, when used, adds a couple of useful pre-made custom completions - warp [2000-03-11]

=item * The 'cd' builtin now has a (currently unlimited) history list, accessible using cd -/+/%number. The new builtin 'dirs' displays the stack - warp [2000-03-13]

=item * On Unix, support for a global pshrc file at /etc/pshrc - warp [2000-03-13]

=item * New variable @Psh::executable_noexpand - warp [2000-03-13]

=item * New variable, $Psh::interactive which is set if psh is currently in interactive mode - warp [2000-03-14]

=item * New builtin 'function' which is just a dummy so bash rc files won't complain - warp [2000-03-14]

=item * New builtin 'firsttime' which is a firsttime user setup utility for the .pshrc file. It cannot do a lot currently but will grow in future - warp [2000-03-14]

=item * New builtin 'complete' which adds a mightier and bash compatible way to define custom completions - Hiroo Hayashi [2000-04-11]
...
=item * Fixed some small problems with globbing code - warp [2000-03-09]

=item * Fixed problem with pod2man and CHANGES.pod reported on Sourceforge - warp [2000-03-11]

=item * SIGTERM and HUP is now not ignored anymore and exits the Perl Shell instead  - warp [2000-03-11]

=item * switch -r now reserved for restricted mode, new switch -f for alternative resource file - warp [2000-03-12]

=item * Fixed problem with quoting which was not removed for the executable strategy - warp [2000-03-12]

=item * Builtin unalias does not remove builtins of the same name anymore - warp [2000-03-14]

=item * Piping is now possible in aliases - warp [2000-04-01]

=item * The parser now allows nesting aliases - warp [2000-04-01]
