...
Within B<psh>, the C<-w> flag and 'C<use strict>' are not employed so
that the user is not bound by their stipulations.  Setting C<$^W = 1>
will turn on warnings, and calling 'C<use strict>' will (almost) do
the usual thing if called by the user (see LIMITATIONS, below).
...
Multiline input may be entered in one of two ways: If an input line
contains the Perl "here document" construct as in C<E<lt>E<lt>XXX>,
(anywhere on the line), then input is read and accumulated until
C<XXX> occurs on a line by itself. Then the accumulated input is
processed as if it were a single line. This makes processing of code
containing Perl "here documents" behave as one would expect from Perl
itself. Alternatively, if an input line ends with a '{' character,
then input is read up to a line that contains only a '}', without any
leading whitespace. This makes it easy to define subroutines, as in
...
This brace mechanism for accumulating input is admittedly a hack; a
better mechanism that could be implemented would be to recognize when
the input has an unmatched open brace, and then continue to read input
until braces match.
...
If the input line begins with a '!' character, strip the '!' and send
the remainder to system.
...
to eval. Commas may be automatically inserted between arguments if
needed, depending on the $psh::perlfunc_insert_commas configuration
variable, see below.
...
the $psh::executable_interpolate_arguments configuration variable is
true. 
...
C<qw(comment bang built_in eval)>. 
If you want B<psh> to act more shell-like so that it
automatically recognizes commands like "mv", you should insert the 
'executable' strategy in there somewhere
before 'eval'. Having done that, if you want
"print" to mean the perl print function instead of, say, 
F</usr/bin/print>, you can add in 'perlfunc' before 'executable'.
...
built-in as opposed to simpley defining a procedure main::foo and
...
=item * C<alias NAME REPLACEMENT> [or C<alias NAME=REPLACEMENT>]


Add NAME as a built-in so that NAME <REST_OF_LINE> will execute
exactly as if REPLACEMENT <REST_OF_LINE> had been entered. For
example, one can execute "C<alias ls ls -F>" to always supply the C<-F>
...
builtin before every execution.
...
will be used. If B<-r> is not specified andf the current directory is
...
will be read and executed and then psh will execute. Otherwise, B<psh>
...
user in the B<psh> package for configuration or utility
...
=item $psh::executable_interpolate_arguments (0)
...
=item $psh::history_file ("$ENV{HOME}/.${bin}_history")
...
=item $psh::perlfunc_insert_commas (0)


If this variable is true, then on command lines interepreted by the
perlfunc strategy, commas will be inserted between the arguments as
long as the command line contains no parens, braces, or commas. For 
example, 'print "hello" $dolly' will be sent to eval as 'print
"hello",$dolly'.
...
A similar array of month names.


=item @psh::strategies ( comment, bang, built_in, eval)
...
These functions are called whenever B<psh> wants to produce B<-d>-mode 
...
Interrupt (Control-C) handling is rudimentary, and other
signal-handling is nonexistent.
...
strategies, improved interrupt handling, &psh::evl, $psh::echo, more extensive
