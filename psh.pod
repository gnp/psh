...
to which the TAB completion should apply. The value is a list reference
with two values, the first one the character which should be appended
after the completion is successful, the second value is again a list
reference containing all possible completions. This is probably complicated
