...
New builtin command B<modules> to display a list of loaded Perl modules

=item *

First attempt at a B<ulimit> command.

=item *

New builtin B<printenv> to display a list of environment variables (like
B<env>) or the value of a certain environment variable.
...
New optional strategy B<darwin_apps> - tries to locate Darwin/Mac OS X
...
The C<@Psh::mon> and C<@Psh::wday> variables were removed. Please see the
perldoc for Psh::Locale
