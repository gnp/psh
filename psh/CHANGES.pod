...
=head2 11 Jan 2000 - warp

New features:

=over 4

=item * Windows titles (PSH_TITLE and $Psh::window_title) behave like prompt
strings now.

=item * Perlfunc strategy is now again a default strategy. $Psh::perlfunc_builtins determines wether Perl builtins should be evaled (default 0)

=item * Builtin 'kill' now supports sending a signal to a name job (e.g. "emacs & ; kill emacs")

=item * Added builtin 'strategy' for loading strategies on demand and adding them to the list of strategies to use.
...
=item * Aliases are now hardcoded into parser again so they are properly pipable

=item * Fallback builtins and most other builtins are loaded on demand

=item * Made Psh::symbols() a builtin

=item * Some less used strategies are loadable on demand using builtin 'strategy'

=back

=head2 04 Jan 2000 - warp

Support for window titles. New environment variable PSH_TITLE,
$Psh::change_title
