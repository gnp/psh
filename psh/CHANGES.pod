...
New builtin "set".  Acts like bash set and returns environment.  Could be
...
Changed the Unix resize handler so that it does what is intended from the
code not what it says.  This now works for me in big xterms.
...
Strategies "fallback_builtin","perlfunc" modularized and moved into Psh::Strategy::...

=item *

Perlfunc configuration variables are now in Psh::Strategy::Perlfunc for cleanness - sorry for that annoying change.

=item *

Removed &Psh::news() as I think there's no need for it being included in psh just for the tryme script.
...
=head1 0.007 [2000-01-24]

Contributions primarily by Markus Peter (C<warp>).
