...
=item C<$Psh::prompt_cont> ('> ') [late]

Just like C<$Psh::prompt>, but for continuation lines when multi-line
input is being read.

=item C<$Psh::result_array> ('Psh::val') 
...
=item C<$Psh::which_regexp> ('^[-a-zA-Z0-9_~+]*$')
...
=item C<@Psh::Completion::netprograms> ( B<ping>, B<ftp>, B<ncftp>, B<ssh>, etc. )

Contains names of a number of net based programs for enabling
TAB completion of hostnames/bookmarks

=item C<@Psh::Completion::bookmarks> ( from /etc/hosts )
...
=item C<%Psh::perl_builtins> (by default, all Perl builtins and keywords)
...
=item C<%Psh::perl_builtins_noexpand> (by default, Perl flow-control keywords)
...
=item C<%Psh::Completion::custom_completions>
...
code, more extensive installation instructions, and other minor updates.
