...
=item * C<perlfunc> (not enabled by default)

If the first word of the input line matches either a built-in Perl
function (as determined by the %Psh::perl_builtins hash), or the name
of defined Perl subroutine, pass the line
...
B<PSH_TITLE> - If set and psh runs within a terminal it knows and which
supports changing window titles, then psh will set the window title to
the value of this variable. Note that this is modified automatically
if $Psh::change_title is true.
